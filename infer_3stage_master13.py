"""
infer_3stage.py — 3-Stage 推理 (使用 checkpoints 中 stage1~3)

Stage1: 音频+节拍 → 谱面骨架
Stage2: +骨架 → hold 持续时间 (自回归)
Stage3: +骨架 → slide 路径 (自回归)

输出: 完整 simai 谱面文件 (不含 break/ex 标记)
"""
...
def infer_3stage(
    ...
    # 加载 path → best_timing 映射
    timing_map_path = Path(data_dir) / "slide_path_timing_map.json"
    if timing_map_path.exists():
        path_best_timing = json.loads(timing_map_path.read_text("utf-8"))
    else:
        path_best_timing = {}
    ...
    bpm = bl.bpm
    subdiv = 32
    measure_dur = 4 * 60.0 / bpm   # 1小节 = 4拍, 与subdiv无关
...
if __name__ == "__main__":
    ...
    infer_3stage(
        ...
        output_path="samples/人是猫/maidata_3stage_Master13_v2.txt",
        difficulty="Master", level=13.0, designer="AI",
        device=device,
    )
